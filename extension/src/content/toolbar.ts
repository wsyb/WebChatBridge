...
export class Toolbar {
  ...
  private toggleDropdown(): void {
    if (this.dropdownOpen) { this.closeDropdown(); } else { this.openDropdown(); }
  }
  ...
  private toggleTaskPanel(): void {
    if (this.taskPanelOpen) { this.closeTaskPanel(); } else { this.openTaskPanel(); }
  }
  ...
  showStatus(status: 'idle' | 'generating' | 'executing' | 'injecting' | 'clicking' | 'waiting', toolName?: string): void {
    ...
    switch (status) {
      ...
      case 'executing':
        if (!this.originalTitle) this.originalTitle = titleEl.textContent || 'Web Chat Bridge';
        titleEl.textContent = toolName ? `执行: ${toolName}` : '执行中';
        titleEl.style.color = 'rgba(251, 191, 36, 0.95)';
        break;
      case 'injecting':
        if (!this.originalTitle) this.originalTitle = titleEl.textContent || 'Web Chat Bridge';
        titleEl.textContent = '注入中';
        titleEl.style.color = 'rgba(251, 191, 36, 0.95)';
        break;
      case 'clicking':
        if (!this.originalTitle) this.originalTitle = titleEl.textContent || 'Web Chat Bridge';
      ...
      case 'waiting':
        if (!this.originalTitle) this.originalTitle = titleEl.textContent || 'Web Chat Bridge';
        titleEl.textContent = '等待回复';
        titleEl.style.color = 'rgba(156, 163, 175, 0.95)';
        break;
    }
  }

  updateGeneratingProgress(charCount: number): void {
    const titleEl = this.shadow?.querySelector('.title') as HTMLElement;
    if (!titleEl) return;
    if (!this.originalTitle) this.originalTitle = titleEl.textContent || 'Web Chat Bridge';
    titleEl.textContent = `输出中 (${charCount}字)`;
    titleEl.style.color = 'rgba(96, 165, 250, 0.95)';
  }
