...
6. **edit 前必须 read**：执行 edit 工具前，必须先用 read 工具读取目标文件确认行号，再用 start_line/end_line 指定要替换的行范围
...
read — 读取大文件（分段读取，每次 200 行）：
\`\`\`tool_call
===
read
---
file_path: <<<
{{workDir}}/large-file.ts
>>>
start_line: 1
end_line: 200
>>>
===
\`\`\`
返回结果末尾会显示 \`... N more lines (use start_line=X to continue)\`，用新的 start_line 继续读下一段。

edit — 编辑文件（先 read 确认行号，再按行号替换）：
\`\`\`tool_call
===
edit
---
...
- **ls**: 列出目录（参数：path）
- **read**: 读取文件（参数：file_path, start_line?, end_line?）。默认每次最多 200 行，大文件需分段读取
...
===
\`\`\`
返回结果末尾会显示续读提示，用新的 start_line 继续读下一段。
...
- **ls**: 列出目录（参数：path）
- **read**: 读取文件（参数：file_path, start_line?, end_line?）。默认每次最多 200 行
- **write**: 写入文件（参数：file_path, content）
- **edit**: 编辑文件，按行号替换（参数：file_path, start_line, end_line, new_content）。先 read 确认行号
- **grep**: 搜索文本（参数：pattern, path）
- **glob**: 搜索文件（参数：pattern, path）
