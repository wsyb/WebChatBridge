...
impl Tool for TaskLogsTool {
    ...
    async fn execute(&self, input: Value, _ctx: ToolContext) -> Result<ToolOutput> {
        ...
        let status = task.map(|t| &t.status).unwrap_or(&super::task_manager::TaskStatus::Exited);
        let exit_code = task.and_then(|t| t.exit_code);
        let failed = matches!(status, super::task_manager::TaskStatus::Exited) && exit_code != Some(0);
