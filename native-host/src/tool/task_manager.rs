...
impl TaskManager {
    ...
    pub fn list_tasks(&self, include_exited: bool) -> Vec<TaskEntry> {
        ...
        self.tasks.values().cloned().filter(|t| {
            // 只保留 running 任务
    ...
    pub fn cleanup_old_tasks(&mut self) {
        let to_remove: Vec<String> = self.tasks.iter().filter(|(_, t)| {
            t.status != TaskStatus::Running
    ...
    /// 读取任务日志
    pub fn get_logs(&self, id: &str, tail: Option<usize>, max_bytes: Option<usize>) -> Result<(String, String)> {
    ...
    pub async fn kill_task(&mut self, id: &str) -> Result<(TaskEntry, String)> {
        ...
        let entry = self.tasks.get(id).cloned()
...
fn read_log_file(path: &Path, tail: Option<usize>, max_bytes: Option<usize>) -> Result<String> {
    ...
    if file_size > limit || total_lines > n {
        ...
        if truncated_bytes {
            hint = format!("[truncated: showing last {} bytes of {} bytes total]", limit, file_size);
